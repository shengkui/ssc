...
*
* DESCRIPTION:
*     The example of client using stream socket(TCP).
*
* REVISION(MM/DD/YYYY):
...
void print_usage(char *pname)
{
    printf("\n"
        "================================================\n"
        "     Client to communicate via stream socket    \n"
        "                    v%d.%d                      \n"
        "================================================\n"
        "\n"
...
int main(int argc, char *argv[])
{
    ...
    clnt = client_init(server_ip, serv_port, 10);
    if (clnt == NULL) {
    ...
    /********************** Get version of server ***********************/
    {
        ...
        ver = (ssc_response_version_t *)client_send_request(clnt, &req);
        if (ver == NULL) {
    ...
    /********************** Get message from server ***********************/
    {
        ...
        res = (ssc_response_get_msg_t *)client_send_request(clnt, &req);
        if (res == NULL) {
            printf("Error: client send request error\n");
            client_close(clnt);
            return STATUS_ERROR;
    ...
    /********************** Send an unknown request to server ***********************/
    {
        ...
        res = (ssc_command_t *)client_send_request(clnt, &req);
        if (res == NULL) {
            printf("Error: client send request error\n");
            client_close(clnt);
            return STATUS_ERROR;
